...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--queries",
        nargs="*",
        help="Queries to search for (default: all configured queries)",
    )
    ...
    parser.add_argument("--delay", type=float, help="Override finder delay in seconds")
...
def check_output_safety(output_dir: str, output_config) -> Tuple[bool, List[str]]:
    ...
    file_bases = [
        output_config.companies_file,
        output_config.domains_file,
        output_config.emails_file,
    ]
    ...
    try:
        ...
        if not is_safe and existing_files:
            if args.fresh:
                ...
                logger.info(
                    "📁 Existing output files detected - running in RESUME mode:"
                )
                for file_info in existing_files:
        ...
        if args.fresh and existing_files:
            ...
            file_extensions = ["txt", "csv", "jsonl", "json", "xlsx"]
            file_bases = [
                config.output.companies_file,
                config.output.domains_file,
                config.output.emails_file,
            ]
            ...
            if any(stats.values()):
                logger.info(
                    f"   📊 Found: {stats['companies']} companies, {stats['domains']}"
                    f" domains, {stats['emails']} emails"
                )
        if args.queries:
            if config.queries:
                config.queries = args.queries
                config.queries = list(set(config.queries))  # deduplicate
                logger.info(f"Extended queries with CLI args: {args.queries}")
            else:
                config.queries = args.queries
                logger.info(f"Using CLI queries: {args.queries}")
