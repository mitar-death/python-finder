        ...
        try:
            ...
            for business in businesses:
                ...
                if yelp_url and name:
                    business_domain = self.domain_resolver.extract_business_domain(
                        company_name=name,
                        yelp_url=yelp_url,
                        address=address
                    )
                ...
                valid_domain = None
                if business_domain and self.domain_resolver._is_valid_business_domain(business_domain):
                    valid_domain = business_domain
                
                company = Company(
                    name=name,
                    url=yelp_url,  # Keep Yelp URL for reference
                    domain=valid_domain,  # Only valid business domains
                    address=address,
